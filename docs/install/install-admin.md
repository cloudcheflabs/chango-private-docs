...
Download Chango Private distribution.
```agsl
curl -L -O https://github.com/cloudcheflabs/chango-libs/releases/download/chango-private-deps/chango-private-2.2.0.tar.gz
```

And tar the file and move to the installation directory of Chango Private.

```agsl
tar zxvf chango-private-2.2.0.tar.gz 
cd chango-private-2.2.0/ansible/
```
...
cpadmin_ansible_path=/data/chango/cp-dist/chango-private-2.2.0/ansible \
run_option=restart \
cpadmin_nginx_scheme=https \
cpadmin_nginx_host=[your-nginx-host] \
cpadmin_nginx_port=[your-nginx-port] \
